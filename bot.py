...
ADMIN_ID = 7408327565 # الـ ID الخاص بك كمدير
...
# رسالة الترحيب وحفظ المستخدم الجديد
...
# --- 3. ميزة الإذاعة (للمدير فقط) ---
async def send_all(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id != ADMIN_ID:
        await update.message.reply_text("⛔️ عذراً، هذا الأمر مخصص للدكتور ياسين فقط.")
        return
    ...
    for u in users:
        try:
            await context.bot.send_message(chat_id=u, text=f"📢 رسالة من د. ياسين:\n\n{message}")
            count += 1
        except: pass
    
    await update.message.reply_text(f"✅ تم إرسال رسالتك بنجاح إلى {count} مشترك.")

# --- 4. نظام التذكير التلقائي (الصباح والمساء) ---
...
if __name__ == '__main__':
    ...
    # ضبط التوقيت على توقيت الجزائر (UTC+1)
    tz_algeria = datetime.timezone(datetime.timedelta(hours=1))
    
    # جدولة رسالة الصباح (07:00) ورسالة المساء (17:00)
